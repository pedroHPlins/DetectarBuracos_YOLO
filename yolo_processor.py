...
def extract_detection_data(predictions):
    ...
    Args:
        predictions: Lista de dicionários com as predições (cada uma contendo 'box', 'confidence', 'class').
    ...
    detections = []
    if not predictions:
        print("Lista de predições vazia para extrair dados.")
        return detections

    for p in predictions:
        try:
            class_id = p.get('class_id', 0) # Roboflow API pode não retornar class_id diretamente, assumindo 0 para 'pothole'
            class_name = p.get('class', 'pothole') # Roboflow API retorna 'class'
            confidence = p.get('confidence')
            box = p.get('box')
            
            if all(v is not None for v in [confidence, box]):
                detections.append({
                    "class_id": class_id,
                ...
                print(f"Predição incompleta, pulando: {p}")
        except Exception as e:
            print(f"Erro ao processar predição {p}: {e}")
            continue
